...
// set_up: {{{
    ...
    int d = 12,
        Nk = 16, // L = Nk * a0 is the size of the system
        Nomega = pow(2, d);
    ...
    double lambda = 1, // interpolation parameter
           nd = 0.75, // density
           tau = 0.057, // natural Temperature tau = kb * T in units of t
           t = 1.0, // 1st nearest neighbor hopping parameter
           tp =  -0.3 * t, // 2nd nearest neighbor hopping parameter
           tpp = 0.0 * t, // 3rd nearest neighbors hopping param
           J = 0.17 * t, // exchange parameter
           gamma_SE = nd / 2; // gamma self-energy


    double mup_initial = -1, // initial guess for the chemical potential 
           u0_initial = 1.5; // initial guess for the second chemical potential

    int neta = 7; // width of the Lorentzian: GAMMA = pow(2, neta) * DELTAomega

    double omega_domain = 20 * t,
           omegai = -omega_domain,
           omegaf = omega_domain,
