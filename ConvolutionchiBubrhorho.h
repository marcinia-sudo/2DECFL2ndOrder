...
#ifndef _CONVOLUTIONCHIBUBRHORHO_H
#define _CONVOLUTIONCHIBUBRHORHO_H
...
void product_chiBubrhorho(void *in1,
                          void *in2,
                          void *out,
                          size_t *N)
{
    ...
    for (i = 0; i < N[0]; i++) {
        for (j = 0; j < N[1]; j++) {
            for (k = 0; k < N[2]; k++) { // (Npad[2] / 2 + 1)
                n = k + N[2] * (j + N[1] * i);
                // abar (bf) - (af)bar b
           //     product[n] = conj(afermi[n]) * b[n] - afermi[n] * conj(b[n]);
                 product[n] = (conj(b[n]) * afermi[n] - conj(afermi[n]) * b[n]) * pow(-1, k);
            }
        }
    }
}

struct ChiBubrhorhoParams
{
    size_t *N,
           *padFT,
           *NpadFT;
...
MKL_INT chiBubrhorho_function(void *params)
{
    ...
    struct ChiBubrhorhoParams *p = (struct ChiBubrhorhoParams *) params;
    size_t *N = p->N,
           *padFT = p->padFT,
           *NpadFT = p->NpadFT;
    ...
    double *fermi = p->Fermi_array,
           *rho_bigG = p->rho_bigG,
           *rho_chiBubrhorho = p->rho_chiBubrhorho;
    ...
    if (PRINT) {
        printf("%zu %zu %zu %zu %zu\n", Ns, N3, N3halfcmplx, N3padFT, N3halfreal);
    }
    ...
    product_chiBubrhorho(rho_bigG_Fermi_forward, rho_bigG_forward, out1, Nhalfcmplx);
